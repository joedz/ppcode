...
bool Poller::stopping(){
    return m_state == TaskState::stop;
}
  ...
  void Poller::networkPoller() {
    ...
    std::shared_ptr<struct epoll_event> shared_event(eventArray, 
        []( epoll_event* ptr){
            delete ptr;
        });
    ...
    while(true) {
        ...
        //LOG_INFO(g_logger) << "timeout and m_state=" << ToStringTaskState(m_state) 
        ...
        for(int i = 0; i < rt; ++i) {
            ...
            if(rt) {
                LOG_ERROR(g_logger) << "epoll_ctl del"<< fd_ctx->fd_ << "error"; 
                continue;
            }
    ...
    LOG_ERROR(g_logger) << "The network pollster has stopped and there are now "<<
     m_pendingEventCount <<" more events being listened on."; 
