...
协程执行器状态
    ...
    运行状态    // 执行器正在运行
    阻塞状态    // 其他执行器可窃取本执行器协程
    idle状态    // 空闲状态 可优先获取协程  此时等待条件变量
并且此时可以去窃取其他执行器的协程 stop        // 停止状态
*/
...
public:
    ...
    static void free(void* ptr, size_t size) { return ::free(ptr); }
};
...
public:
    ...
    // 切入协程执行
    INLINE void SwapIn() { jump_fcontext(&GetTlsContext(), m_ctx, m_vp); }

    // 切出协程执行
    INLINE void SwapOut() { jump_fcontext(&m_ctx, GetTlsContext(), m_vp); }
