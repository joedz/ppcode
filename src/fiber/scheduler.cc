...
void Scheduler::start(){
    ...
    for(size_t i = 0; i < m_processerNumber; ++i) {
        ...
        m_procThreads.push_back(
            std::make_shared<Thread>(thrName, std::bind(&Processer::execute, proc))
        );
        m_processers.push_back(proc);
    }

    m_poller = new Poller(this);
    m_threadPoller =  std::make_shared<Thread>("Network poller", std::bind(&Poller::networkPoller, m_poller));
...
void Scheduler::addFiber(Fiber::ptr fb){
    ...
    // 查看协程之前是否有P 如果有那么将协程加入到P的执行队列  当前P处于活跃状态 那么将很快执行这个协程
   ...
   for(auto& it : m_processers) {
    ...
    // TODO 如果说当前执行器都出与阻塞状态, 那么向阻塞的执行器中加入协程,协程将不能直接运行,是否可以先将协程加入到一个全局队列里面
    auto& procSptr = m_processers[id];
    procSptr->addFiber(fb);
    id = (id + 1) % n;
