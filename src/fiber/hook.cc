...
static ssize_t do_io(int fd, OriginFun fun, const char* hook_fun_name, uint32_t event, int timeout, Args&&... args){
    ...
    while(true) {
        ...
        if(n == -1 && errno == EAGAIN) {
            ...
            if(to != (uint64_t)-1) {
                // 设置超时时间
                timerTask = sche->getPoller()->createCondTimer([winfo, fd, sche, event](){
                    std::shared_ptr<timer_info> t= winfo.lock();
                    if(!t || t->canclelled) {
                        return;
                    }
                    t->canclelled = ETIMEDOUT;
                    sche->getPoller()->cancelEvent(fd, (ppcode::FdContext::Event)event);
                }, winfo, to);
            }
            // 添加事件监听 
            int rt = sche->getPoller()->addEvent(fd, (ppcode::FdContext::Event)event);
            if(rt) {
                // epoll_ctl添加错误
                if(timerTask) {
                    timerTask->cancel();
...
// nanosleep 函数 使当前线程挂起一段时间 eq->tv_sec是以秒为单位，而tv_nsec以纳秒为单位（10的-9次方秒）
...
int connect_with_timeout(int sockfd, const struct sockaddr *addr, socklen_t addrlen, uint64_t timeout);
 ...
 int connect(int sockfd, const struct sockaddr *addr, socklen_t addrlen){
    ...
    return connect_with_timeout(sockfd, addr, addrlen, ppcode::s_connect_timeout);
 }
...
ssize_t readv(int fd, const struct iovec *iov, int iovcnt) {
    return do_io(fd, readv_f, "readv", ppcode::FdContext::READ, SO_RCVTIMEO, iov, iovcnt);
}

ssize_t recv(int sockfd, void *buf, size_t len, int flags) {
    return do_io(sockfd, recv_f, "recv", ppcode::FdContext::READ, SO_RCVTIMEO, buf, len, flags);
}

ssize_t recvfrom(int sockfd, void *buf, size_t len, int flags, struct sockaddr *src_addr, socklen_t *addrlen) {
    return do_io(sockfd, recvfrom_f, "recvfrom", ppcode::FdContext::READ, SO_RCVTIMEO, buf, len, flags, src_addr, addrlen);
}

ssize_t recvmsg(int sockfd, struct msghdr *msg, int flags) {
    return do_io(sockfd, recvmsg_f, "recvmsg", ppcode::FdContext::READ, SO_RCVTIMEO, msg, flags);
}
...
ssize_t write(int fd, const void *buf, size_t count) {
    return do_io(fd, write_f, "write", ppcode::FdContext::WRITE, SO_SNDTIMEO, buf, count);
}

ssize_t writev(int fd, const struct iovec *iov, int iovcnt) {
    return do_io(fd, writev_f, "writev", ppcode::FdContext::WRITE, SO_SNDTIMEO, iov, iovcnt);
}

ssize_t send(int s, const void *msg, size_t len, int flags) {
    return do_io(s, send_f, "send", ppcode::FdContext::WRITE, SO_SNDTIMEO, msg, len, flags);
}

ssize_t sendto(int s, const void *msg, size_t len, int flags, const struct sockaddr *to, socklen_t tolen) {
    return do_io(s, sendto_f, "sendto", ppcode::FdContext::WRITE, SO_SNDTIMEO, msg, len, flags, to, tolen);
}

ssize_t sendmsg(int s, const struct msghdr *msg, int flags) {
    return do_io(s, sendmsg_f, "sendmsg", ppcode::FdContext::WRITE, SO_SNDTIMEO, msg, flags);
}
 ...
 int connect_with_timeout(int sockfd, const struct sockaddr *addr, socklen_t addrlen, uint64_t timeout){
    ...
    if(timeout != (uint64_t)-1) {
        //创建条件定时器 
        timerTask = sche->getPoller()->createCondTimer([winfo, sockfd, sche](){
            auto t = winfo.lock();
            if(!t || t->canclelled) {
                return;
            }
