...
TimerTask::TimerTask(std::function<void()> cb, uint64_t ms, Timer* timer, bool recurring)
:m_cb(cb),
 m_ms(ms),
...
bool TimerTask::Comparator::operator()(const TimerTask::ptr& lhs, const TimerTask::ptr& rhs) const{
...
TimerTask::ptr Timer::createTimer(const std::function<void()>& cb, uint64_t ms, bool recurring){
...
TimerTask::ptr Timer::createCondTimer(const std::function<void()>& cb, std::weak_ptr<void> weak_cond,uint64_t ms, bool recurring){
