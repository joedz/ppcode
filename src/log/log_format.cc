...
void LogFormatter::parserFormatter(const std::string& pattern) {
    ...
    for (; i < pattern.size(); ++i) {
        if (pattern[i] == '%') {
            ...
            switch (pattern[i]) {
                ...
                case 'D':  //时间 time   %D{}
                    if (j < pattern.size() && pattern[j] == '{') {
                        ...
                        if (j >= pattern.size() || pattern[j] != '}') {
                            m_error = true;
                            std::cout << "log patrern: May be missing \"}\""
                                      << std::endl;
                            return;
                        }
                        items.push_back(std::make_shared<TimeFormatter>(
                            pattern.substr(i + 2, j - i - 2)));
                        i = j;
                    } else {
                        items.push_back(std::make_shared<TimeFormatter>());
                    }
                ...
                case 't':  // 线程id
                    ...
                    break;
                default:  // 错误
                    std::cout << "log patrern: unkown pattern is %"
                              << pattern[i] << std::endl;
                    m_error = true;
                    return;
            }  // end switch
            ...
            items.push_back(std::make_shared<StringFormatter>(
                pattern.substr(i, j - i + 1)));
            i = j;
        }
